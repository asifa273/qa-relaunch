...
test.only('Browser Client Register ', async ({ browser }) => {
  ...
  //Register link
  // await expect(page.getByRole('heading', { name: 'We Make Your Shopping Simple' })).toBeVisible();
  // await expect(page.getByRole('heading', { name: 'Register' }));
  // // or a field:
  // await expect(page.locator('#firstName')).toBeVisible();
  // await expect(page.getByRole('heading', { name: 'We Make Your Shopping Simple' })).toBeVisible({ timeout: 15000 });
  // console.log(await page.getByRole('heading', { name: 'We Make Your Shopping Simple' }).count());
  ...
  await page.pause();          // opens Inspector → paste the locator in its Locator box
  console.log(await (registerButton).textContent());
  await registerButton.click();
  await page.locator('#firstName').fill('Testbbclmq');
  await page.locator('#lastName').fill("7849991");
  await page.locator('#userEmail').fill("Testbbclm7849991@ymail.com");
